...
export default function CrewManifest({ alertMode }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        {CREW_MANIFEST.map(member => (
          <button
            ...
            className={`w-full text-left px-4 py-2.5 border-b ${borderColor} ${hoverBg} transition-colors group`}
          >
              ...
              <div>
                <span
                  className={`text-sm transition-colors ${
                    member.isEduardo
                      ? 'text-console-amber group-hover:text-console-phosphor-bright'
                      : member.deceased
                      ? 'text-console-red line-through opacity-60 group-hover:no-underline'
                      : `${textColor} group-hover:text-console-phosphor-bright`
                  }`}
                >
                  {member.name}
                </span>
      ...
      <div className={`px-3 py-2 border-t ${borderColor} ${dimColor} font-console text-[10px]`}>
        CLICK CREW MEMBER TO ACCESS PERSONNEL FILE
      </div>
    </div>
  )
}
